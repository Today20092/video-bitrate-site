...
(function () {
  ...
  function formatUploadSpeed(speedMbps) {
    const value = speedMbps >= 1000 ? speedMbps / 1000 : speedMbps;
    const unit = speedMbps >= 1000 ? 'Gbps' : 'Mbps';
  ...
  function getUploadBarPercent(uploadTime, maxTime) {
    if (!Number.isFinite(uploadTime) || uploadTime <= 0) return 0;
    return Math.max(2, Math.min(100, (uploadTime / Math.max(maxTime, 0.001)) * 100));
  }

  function formatUploadSpeedDetail(speedMbps) {
    return `${formatUploadSpeed(speedMbps)} advertised, ${formatUploadSpeed(speedMbps * uploadEfficiency)} effective`;
  }

  function getUploadSpeedDetail(speedMbps) {
    return formatUploadSpeedDetail(speedMbps);
  }
